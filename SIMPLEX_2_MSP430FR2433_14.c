#include <msp430.h> 
// SIMPLEX 2_MSP430FR2433_14

// SIMPL Interpreter for MSP430FR2433 ChipStick V2.14

// - inspired by Txtzyme Nano Interpreter -  an original idea by Ward Cunningham

// An on going Neo-Retro Computing Mission!

// Ken Boak  March 29th 2016

// This version works with 23LC1024  128Kbyte SRAM
//
// Includes LED support for P2.0, P2.1, P2.2 & P2.3
// and P2.4, P2.5 and P2.6 - if you are not using UCA1 

// Using pin toggle t command and pin write w command

// Modified 18-3-2016 to run on MSP430FR2433 Module and on 23-3-2016 to run on ChipStick

// This version adds 23LC1024 128Kx8 external SPI RAM, plus hex-dump utility

// Now has musical notes A to G 


/*
 Module Connections / Pin-Out
                   +------U------+
1             3V3  |  CHIPSTICK  | 0v                20
2             P3.2 |             | P2.1 LED1 XIN     19
3     PC-RX   P1.5 |             | P2.0 LED0 XOUT    18
4     PC-TX   P1.4 |             | TEST/SBWTCK       17
5  UCA0 CLK   P1.6 |             | /RST/NMI/SBWTDIO  16
6     LED3    P2.3 |             | P2.6 UCA1 MOSI    15
7  RAM_/CS    P3.0 |             | P2.5 UCA1 MISO    14
8  UCB0 STE   P1.0 |             | P2.4 UCA1 CLK          13
9  UCB0 SCK   P1.1 |             | P2.2 LED2         12
...
#define        WP_PIN          BIT3                   // write protect pin P2.3
...
#define        wpSelect        P2OUT &= ~WP_PIN
#define        wpDeselect      P2OUT |=  WP_PIN
...
void GPIO_init_2433()
{
  ...
  P1OUT = 0x01; P2OUT = 0xFF; P3OUT = 0x17;

  // Configure GPIO

//  P2DIR |= BIT3;                          // For /FRAM_WP
//  P2OUT |= WP_PIN;  						// /FRAM_WP high
...
/*
      case 'i':
      x = digitalRead(x,y);        //   read pin p
      break;


      case 'o':
      digitalWrite(d, x%2);     //  write to pin p
      break;
*/
      case 's':
//      x = ADC_Read();         //
...
/*
      case 'w':
 //     analogWrite(d,x);           // PWM write to pin p
      break;
...
// Launchpad LED group  support for red and green LEDs on entry level LaunchPad
      ...
      case 'w':
 {
 digitalWrite(y,x);      // Set / reset the digital pin on Port 2
 }
 break;
