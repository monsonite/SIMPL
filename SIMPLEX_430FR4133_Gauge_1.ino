...
// MSP430FR4133 UART, SPI and LCD support Routines
// A 6 six digit number can be sent to the display using the 123456n command

// - codesize is now 4892bytes
...
// Note Debug UART is running at 125Kbaud - note the unusual value.
...
// Timing & Printing Group
      ...
      case '.':                           // print a terminating OK  - if desired
      print_ok();    
      break;
...
//     ADC_poll();         // Poll the Loadcell ADC with "s", wait for result and display
     ...
     print_ok();          // confirm after 100 samples
      break;
...
void LCD_display_n(long number)

{
  ...
  LCDMEMCTL |= LCDCLRM;                                      // Clear LCD memory
   ...
   if(mod>=999999) {mod = 999999;}                           // Do not exceed 6 digits on display
   j = 100000;
   
   for(int i = 1; i <=6; i++)
   {
     n[i] = mod / j;                                        // Extract the digits in turn
     mod = mod  % j;
     j = j /10;
   }  
   ...
   */
    ...
    LCDMEM[pos1] = digit[n[1]]; 
    LCDMEM[pos2] = digit[n[2]];   
    LCDMEM[pos3] = digit[n[3]];     
    LCDMEM[pos4] = digit[n[4]];    
    LCDMEM[pos5] = digit[n[5]];       
    LCDMEM[pos6] = digit[n[6]];
